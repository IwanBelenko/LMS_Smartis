...
и проверяет внутренний и публичный health endpoint. Remote-скрипт сначала
полностью загружается на сервер и только затем запускается из файла, чтобы Docker
Compose не мог перехватить его stdin. Финальный SSH-шаг отдельно сверяет commit,
тег и активную ссылку релиза.
